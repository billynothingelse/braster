            ...
            if (p.x >= 0 && p.y >= 0 && p.x < image.width && p.y < image.height) {
                ...
                if (zbuffer[(i32)(p.x + p.y * image.width)] < pz)
                {
                    ...
                    if (dot > 0.0f) {
                        ...
                        color_t tex_color = tga_get_pixel(diffuse, uv);
                        float spec_intensity = (3.f * tga_get_pixel_intensity(specular, uv) * powf(MAX(0.0f, reflected_light.z), 32.0f));

                        u8 spec_b = (u8)(spec_intensity);
                        u8 spec_g = (u8)(spec_intensity);
                        u8 spec_r = (u8)(spec_intensity);

                        color_t col = { 
                            (u8)(tex_color.r * dot + spec_r), 
                            (u8)(tex_color.g * dot + spec_g), 
                            (u8)(tex_color.b * dot + spec_b) 
                        };
